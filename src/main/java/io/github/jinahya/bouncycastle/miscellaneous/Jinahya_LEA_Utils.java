 ...
 */
public final class Jinahya_LEA_Utils {

    // ----------------------------------------------------------------------------------------------- /CBC/PKCS7Padding
    private static JinahyaCrypto _CBC_PKCS7Padding(final byte[] key, final byte[] iv) {
        _LEA___Utils.requireValidKey(key);
        __CBC__Utils.requireValidIv(iv, _LEA___Constants.BLOCK_BYTES);
    ...
    private static JinahyaCrypto _GCM_NoPadding(final byte[] key, final int tLen, final byte[] iv, final byte[] aad) {
        ...
        final var cipher = GCMBlockCipher.newInstance(new LEAEngine());
