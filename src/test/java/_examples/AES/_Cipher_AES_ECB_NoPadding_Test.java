...
@DisplayName(_Cipher_AES___Test.ALGORITHM + '/' + _Cipher_AES_ECB__Test.MODE + "NoPadding")
@NoArgsConstructor(access = AccessLevel.PACKAGE)
@Slf4j
@SuppressWarnings({
        "java:S3577" // Test classes should comply with a naming convention
})
class _Cipher_AES_ECB_NoPadding_Test
        extends _Cipher_AES_ECB__Test {
    ...
    private static Stream<Arguments> getCipherAndKeyArgumentsStream() {
        return PROVIDER_NAME_LIST.stream().flatMap(pn -> KEY_SIZES.stream().map(ks -> {
            final Cipher cipher;
            try {
    ...
    @MethodSource({"getCipherAndKeyArgumentsStream"})
    @ParameterizedTest
    void __(final Cipher cipher, final Key key) throws Exception {
        // ------------------------------------------------------------------------------------------------------- given
    ...
    @MethodSource({"getCipherAndKeyArgumentsStream"})
    @ParameterizedTest
    void __(final Cipher cipher, final Key key, @TempDir final File tempDir) throws Exception {
        // ------------------------------------------------------------------------------------------------------- given
